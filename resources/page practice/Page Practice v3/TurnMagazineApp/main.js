...
async function handleFiles(files) {
    ...
    try {
        // UI Transition BEFORE init so we can calculate size
...
async function loadPDF(file) {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
        ...
        // Set ratio from first page
        if (i === 1) {
            const viewport = page.getViewport({ scale: 1 });
            flipbookRatio = (viewport.width / viewport.height) * 2; // Double page view
        }
...
async function loadImages(files) {
    for (let i = 0; i < files.length; i++) {
        ...
        // Disable ratio check for images for now as getting dims is async
        // Instead assume standard ratio or update if we preload.
        // For simplicity, let's keep the user's ratio if they drop meaningful images.
        // We can create a temp image to check dimensions of the FIRST one.
        if (i === 0) {
            const img = new Image();
            img.src = dataUrl;
            await new Promise(r => img.onload = r); // Wait for load
            flipbookRatio = (img.width / img.height) * 2;
        }
...
// State
let flipbookRatio = 1.6; // Default aspect ratio (960/600)
...
function calculateSize() {
    const margin = 80; // Space for controls (header is handled by flex)
    const maxWidth = container.clientWidth - 40;
    const maxHeight = container.clientHeight - margin;

    let width = maxWidth;
    let height = width / flipbookRatio;
    ...
    // Ensure even width for double page view
